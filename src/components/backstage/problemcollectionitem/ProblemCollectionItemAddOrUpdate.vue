...
<script>
    ...
    export default {
        ...
        methods: {
            async prepareSave(){
                let result = await save("/executor/problem-collection-item",this.newEntity)
                if(result.success){
                    this.$emit("reloadData");
                }
            },
            async prepareUpdate(){
                let result = await update("/executor/problem-collection-item",this.newEntity.id,this.newEntity)
                if(result.success){
                    this.$emit("reloadData");
                }
            }
