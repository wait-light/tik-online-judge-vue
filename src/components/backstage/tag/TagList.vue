<template>
    ...
    <el-dialog title="添加/修改" v-model="prepareEntity.open">
        <div class="dialog-message">
            <TagAddOrUpdate @reloadData="loadData" :entity="prepareEntity.entity"></TagAddOrUpdate>
        </div>
    </el-dialog>
    <el-pagination
...
<script>
    ...
    import TagAddOrUpdate
        from "@/components/backstage/tag/TagAddOrUpdate.vue";
    ...
    export default {
        ...
        methods: {
            ...
            async prepareDelete(entity) {
                let result = await deleteById("/executor/tag", entity.id);
                if (result.success) {
                    this.loadData();
                }
            },
            ...
            async loadData() {
                ...
                let result =await getList("/executor/tag/list",this.pageInfo.page,this.pageInfo.pageSize);
                if(result.success){
