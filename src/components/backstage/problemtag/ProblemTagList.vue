<template>
    ...
    <el-dialog title="添加/修改" v-model="prepareEntity.open">
        <div class="dialog-message">
            <ProblemTagAddOrUpdate @reloadData="loadData" :entity="prepareEntity.entity"></ProblemTagAddOrUpdate>
        </div>
    </el-dialog>
    <el-pagination
...
<script>
    ...
    import ProblemTagAddOrUpdate
        from "@/components/backstage/problemtag/ProblemTagAddOrUpdate.vue";

    export default {
        components: {
            ProblemTagAddOrUpdate,
        },
        data() {
            return {
                table:[],
        ...
        methods: {
            ...
            async prepareDelete(entity) {
                let result = await deleteById("/executor/problem-tag", entity.id);
                if (result.success) {
                    this.loadData();
                }
            },
            ...
            async loadData() {
                ...
                let result =await getList("/executor/problem-tag/list",this.pageInfo.page,this.pageInfo.pageSize);
                if(result.success){
