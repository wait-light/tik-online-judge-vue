<template>
    <el-form v-model="newEntity" label-position="right" label-width="80px">
            ...
            <el-form-item label="标签id">
                <el-input v-model="newEntity.tagId"></el-input>
            </el-form-item>
        <el-form-item>
...
<script>
    ...
    export default {
        ...
        methods: {
            async prepareSave(){
                let result = await save("/executor/problem-tag",this.newEntity)
                if(result.success){
                    this.$emit("reloadData");
                }
            },
            async prepareUpdate(){
                let result = await update("/executor/problem-tag",this.newEntity.id,this.newEntity)
                if(result.success){
                    this.$emit("reloadData");
                }
            }
