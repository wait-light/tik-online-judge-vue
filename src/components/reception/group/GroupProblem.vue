<template>
  <div class="buttons">
    <el-button
      @click="this.$router.push(`/group/${$route.params.groupId}/problem`)"
      type="success"
      size="mini"
    >添加</el-button>
  </div>
    ...
    <el-table-column label="操作" fixed="right" width="240">
      <template #default="scope">
        <el-button
          ...
          @click="
            $router.push(
              `/group/${$route.params.groupId}/problem/${scope.row.id}?secretKey=${scope.row.secretKey}`
            )
          "
          type="warning"
        >修改</el-button>
        <el-button size="mini" @click="prepareDelete(scope.row.id)" type="danger">删除</el-button>
...
export default {
  ...
  methods: {
    ...
    async prepareDelete(problemId) {
      let result = await deleteData(
        `/executor/collection-group/problems/${this.$route.params.groupId}/${problemId}`,
        null,
        true
      );
      if (result.success) {
